var margin_elite = { top: 50, right: 50, bottom: 50, left: 50 };

var width_elite =600 - margin_elite.left - margin_elite.right;
var height_elite = 600 - margin_elite.top - margin_elite.bottom;

var color_elite = d3.scaleOrdinal(d3.schemeCategory10);

// Create SVG element and set dimensions
const svg_elite = d3.select("#chart1").append("svg")
    .attr("width", "100%")
    .attr("height", "100%")
    .attr("viewBox", `0 0 ${700} ${700}`);

// Load the graph data from JSON file
d3.json("data/elite_graph_data.json").then(function(graph) {
    const clusterData = {};
    ...
    node.append("circle")
        .attr("r", 5)
        .attr("fill", d => color_elite(d.group))
    ...
    const simulation = d3.forceSimulation(graph.nodes)
        .force("link", d3.forceLink(graph.links).id(d => d.index).distance(80).strength(link => link.source.group === link.target.group ? 1 : 0.1))
        .force("charge", d3.forceManyBody().strength(-50))  // Repulsive force to spread nodes
        .force("center", d3.forceCenter(width_elite / 2, height_elite / 2))
    ...
    const clusters = svg_elite.append("g")
        .attr("class", "clusters")
        .selectAll("ellipse")
        ...
        .attr("stroke", d => color_elite(d.group))
        .attr("stroke-width", 2);

    function updateClusterLabels() {
        // Remove existing labels and backgrounds
        svg_elite.selectAll("text.cluster-label").remove();
        svg_elite.selectAll("rect.cluster-label-bg").remove();

        // Add cluster labels
        const labels = svg_elite.selectAll("text.cluster-label")
            .data(Object.values(clusterData))
            .enter()
            .append("text")
            ...
            .style("fill", d => color_elite(d.group))
            .text(d => {
        ...
        labels.each(function(d) {
            ...
            svg_elite.insert("rect", ".cluster-label")
